...
//     GROUP BY 
//       s.id, s.name, s.email, s.number, s.address, s.created_at
//     ORDER BY 
//       s.created_at DESC;
//   `, (err, result) => {
//     if (err) throw err;
...
function perfPercent(verified, totalDays) {
  if (!totalDays || totalDays <= 0) return 0;
  return +( (verified / totalDays) * 100 ).toFixed(1);
}
...
// --- main dashboard ------------------------------------------
router.get('/internship', async (req, res, next) => {
  try {
    const { start, end } = ensureDateRange(req.query);
    const qSearch = (req.query.q || '').trim();                  // search by name/number
    const statusFilter = (req.query.status || 'all').toLowerCase(); // all | verified | pending | rejected | missed-today
    const minPerf = Number(req.query.minPerf || 0);              // min performance %
...
ambassadors.forEach((a) => {
  ...
  a.rejected_range = a.rejected_total;
    ...
    const sortMap = {
      created_at: (a, b) => (new Date(a.created_at) - new Date(b.created_at)) * dir,
      name: (a, b) => a.name.localeCompare(b.name) * dir,
      performance: (a, b) => (a.performance_range - b.performance_range) * dir,
      verified_range: (a, b) => (a.verified_range - b.verified_range) * dir,
    };
    filtered.sort(sortMap[sort] || sortMap['created_at']);

    // 2) Today: verified list
    const todayVerifiedSQL = `
  SELECT
    idt.brand_ambassador_id,
    s.name, s.email,
    s.number,
    DATE(idt.created_at) AS on_date,
    idt.status,
    idt.proofLink
...
`;
    ...
    // 3) Today: missed list (eligible today but no verified submission today)
    const todayMissedSQL = `
      SELECT s.id AS brand_ambassador_id, s.name, s.email, s.number, s.created_at
      FROM shopkeeper s
      LEFT JOIN (
        SELECT DISTINCT brand_ambassador_id
        FROM internship_day
    ...
    const weeklySQL = `
      SELECT
        ...
        s.name, s.email,
        s.number,
        SUM(CASE WHEN DATE(idt.created_at) = CURDATE() - INTERVAL 0 DAY AND idt.status='verified' THEN 1 ELSE 0 END) AS d0,
        SUM(CASE WHEN DATE(idt.created_at) = CURDATE() - INTERVAL 1 DAY AND idt.status='verified' THEN 1 ELSE 0 END) AS d1,
        SUM(CASE WHEN DATE(idt.created_at) = CURDATE() - INTERVAL 2 DAY AND idt.status='verified' THEN 1 ELSE 0 END) AS d2,
        SUM(CASE WHEN DATE(idt.created_at) = CURDATE() - INTERVAL 3 DAY AND idt.status='verified' THEN 1 ELSE 0 END) AS d3,
        SUM(CASE WHEN DATE(idt.created_at) = CURDATE() - INTERVAL 4 DAY AND idt.status='verified' THEN 1 ELSE 0 END) AS d4,
    ...
    const timeseriesSQL = `
      SELECT
        ...
        SUM(idt.status='verified') AS verified,
...
// Export main list with current filters (same as UI)
router.get('/internship/export/main.csv', async (req, res, next) => {
  try {
    const qSearch = (req.query.q || '').trim();
    ...
    const sql = `
      SELECT
        s.id AS brand_ambassador_id,
        s.name, s.email, s.number, s.address, s.created_at,
        DATEDIFF(CURDATE() + INTERVAL 1 DAY, s.created_at) AS total_days,

        /* ALL-TIME verified */
        SUM(CASE WHEN idt.status='verified' THEN 1 ELSE 0 END) AS verified_range,

        /* Optional ALL-TIME for reference */
        SUM(CASE WHEN idt.status='pending' THEN 1 ELSE 0 END)  AS pending_total,
        SUM(CASE WHEN idt.status='rejected' THEN 1 ELSE 0 END) AS rejected_total
    ...
    const shaped = rows.map((r) => {
      ...
      return {
        ...
        verified_all_time: r.verified_range,
        pending_all_time: r.pending_total,
        rejected_all_time: r.rejected_total,
        performance_percent_all_time: perf
      };
    });
...
// Export today's verified
router.get('/internship/export/today-verified.csv', async (req, res, next) => {
  try {
    const sql = `
      SELECT 
        idt.brand_ambassador_id,
    ...
    sendCSV(res, `today_verified.csv`, rows, [
      'brand_ambassador_id',
      'name',
      'email',       // ✅ add this
      'number',
...
// Export today's missed
router.get('/internship/export/today-missed.csv', async (req, res, next) => {
  try {
    const sql = `
      SELECT s.id AS brand_ambassador_id, s.name, s.email, s.number, s.created_at
      FROM shopkeeper s
    ...
    sendCSV(res, `today_missed.csv`, rows.map(r => ({
      ...
      number: r.number,
      created_at: toDateOnly(r.created_at)
    })), ['brand_ambassador_id','name','number','created_at']);
  } catch (err) {
    next(err);
  }
});

// Export 7-day heatmap matrix
router.get('/internship/export/weekly-matrix.csv', async (req, res, next) => {
  try {
    const sql = `
      SELECT
        s.id AS brand_ambassador_id, s.name, s.email, s.number,
        SUM(CASE WHEN DATE(idt.created_at) = CURDATE() - INTERVAL 0 DAY AND idt.status='verified' THEN 1 ELSE 0 END) AS d0,
        SUM(CASE WHEN DATE(idt.created_at) = CURDATE() - INTERVAL 1 DAY AND idt.status='verified' THEN 1 ELSE 0 END) AS d1,
        SUM(CASE WHEN DATE(idt.created_at) = CURDATE() - INTERVAL 2 DAY AND idt.status='verified' THEN 1 ELSE 0 END) AS d2,
        SUM(CASE WHEN DATE(idt.created_at) = CURDATE() - INTERVAL 3 DAY AND idt.status='verified' THEN 1 ELSE 0 END) AS d3,
    ...
    sendCSV(res, `weekly_matrix.csv`, shaped, headers);
  } catch (err) {
    next(err);
  }
});
  ...
  try {
    ...
    const rows = await queryAsync(`
      ...
      WHERE 1=1
        ...
        AND bc.id IS NULL
      ORDER BY s.name, s.email, b.sort_order
    `, params);
  ...
  router.get('/benefits/tracker', async (req, res, next) => {
    try {
      ...
      const rows = await queryAsync(`
        ...
        SELECT s.id AS brand_ambassador_id, s.name, s.email,
               IFNULL(vd.verified_days,0) AS verified_days,
               b.id AS benefit_id, b.code, b.title, b.phase, b.day_threshold,
...
router.get('/youtube/analytics', async (req, res, next) => {
  try {
    const { start, end } = req.query; // optional date filters
    const tiles = await service.getSummaryTiles(start, end);
    res.render('youtubedashboard', { start, end, tiles });
  } catch (e) {
    next(e);
  }
});
...
router.get('/api/overall/daily', async (req, res, next) => {
  try {
    const { start, end } = req.query;
    const data = await service.getOverallWatchTimeDaily(start, end);
    res.json(data);
  } catch (e) { next(e); }
});

router.get('/api/overall/monthly', async (req, res, next) => {
  try {
    const { start, end } = req.query;
    const data = await service.getOverallWatchTimeMonthly(start, end);
    res.json(data);
  } catch (e) { next(e); }
});

router.get('/api/url/daily', async (req, res, next) => {
  try {
    const { start, end } = req.query;
    const data = await service.getUrlWatchTimeDaily(start, end);
    res.json(data);
  } catch (e) { next(e); }
});

router.get('/api/url/monthly', async (req, res, next) => {
  try {
    const { start, end } = req.query;
    const data = await service.getUrlWatchTimeMonthly(start, end);
    res.json(data);
  } catch (e) { next(e); }
});

router.get('/api/promoter/daily', async (req, res, next) => {
  try {
    const { start, end } = req.query;
    const data = await service.getPromoterWatchTimeDaily(start, end);
    res.json(data);
  } catch (e) { next(e); }
});

router.get('/api/promoter/monthly', async (req, res, next) => {
  try {
    const { start, end } = req.query;
    const data = await service.getPromoterWatchTimeMonthly(start, end);
    res.json(data);
  } catch (e) { next(e); }
});
