...
router.post('/insert',upload.fields([{ name: 'college_logo', maxCount: 1 }, { name: 'affilated_college_logo', maxCount: 1 }]),(req,res)=>{
    ...
    pool.query(`insert into customize_project set ?`,body,(err,result)=>{
      if(err) throw err;
        ...
        res.redirect('/customization/projects')
    }
    })
})
...
router.post('/insert/synopsis',upload.fields([{ name: 'college_logo', maxCount: 1 }, { name: 'affilated_college_logo', maxCount: 1 }]),(req,res)=>{
    let body = req.body
    body['college_logo'] = req.files['college_logo'][0].filename
    body['affilated_college_logo'] = req.files['affilated_college_logo'][0].filename
    body['date'] = today
    ...
    pool.query(`insert into customize_synopsis_project set ?`,body,(err,result)=>{
      if(err) throw err;
        ...
        res.redirect('/customization/synopsis')
    }
    })
})
...
router.get('/projects',(req,res)=>{
    console.log('why this run',req.session.roll_number)
...
router.get('/synopsis',(req,res)=>{
    if(req.session.roll_number){
    pool.query(`select * from customize_synopsis_project where roll_number = '${req.session.roll_number}' order by id desc limit 1`,(err,result)=>{
        if(err) throw err;
        else {
            console.log(result[0].php)
