...
// Create a transporter for sending emails (config via .env: SMTP_EMAIL_USER, SMTP_EMAIL_PASS)
const smtpUser = process.env.SMTP_EMAIL_USER;
const smtpPass = process.env.SMTP_EMAIL_PASS;
const fromEmail = process.env.SMTP_EMAIL_FROM || smtpUser;

if (!smtpUser || !smtpPass) {
  console.warn('[dataService] SMTP_EMAIL_USER and SMTP_EMAIL_PASS must be set in .env for email features. Emails will fail until configured.');
}
