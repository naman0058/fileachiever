...
(err,result)=>err ? console.log(err) : res.render('ME/index',{result:result,Metatags:onPageSeo.homePage,CommonMetaTags:onPageSeo.commonMetaTags,category:req.categories,fullUrl:req.fullUrl,graduation_type_send:'M.E.',active:'report',navOnly:true}))
})
...
res.redirect(`/me-final-year-project-report-${req.body.seo_name}/edit`)
    }
  ...
  pool.query(`select * from ${table} where roll_number = '${req.session.roll_number}' order by id desc limit 1`,(err,result)=>{
        if(err) throw err;
         ...
         //For Testing
           ...
           pool.query(query+query1+query2+query3+query4,(err,result)=>{
               if(err) throw err;
               else res.render('ME/final',{result:result,navOnly:true})
           })
