...
(err,result)=>err ? console.log(err) : res.render('BE/index',{result:result,Metatags:onPageSeo.homePage,CommonMetaTags:onPageSeo.commonMetaTags,category:req.categories,fullUrl:req.fullUrl,graduation_type_send:'B.E.',active:'report',navOnly:true}))
})
...
router.post('/insert',upload.fields([{ name: 'college_logo', maxCount: 1 }, { name: 'affilated_college_logo', maxCount: 1 }]),(req,res)=>{
    ...
    pool.query(`insert into ${table} set ?`,body,(err,result)=>{
      if(err) throw err;
        ...
        res.redirect('/be-final-year-project-report/projects')
    }
    })
    }
  ...
  pool.query(`select * from ${table} where roll_number = '${req.session.roll_number}' order by id desc limit 1`,(err,result)=>{
        if(err) throw err;
           ...
           pool.query(query+query1+query2+query3+query4,(err,result)=>{
               if(err) throw err;
               else res.render('BE/mobile_view',{result:result})
           })
  ...
  pool.query(`select * from ${table} where roll_number = '${req.session.roll_number}' order by id desc limit 1`,(err,result)=>{
        if(err) throw err;
         ...
         //For Testing
           ...
           pool.query(query+query1+query2+query3+query4,(err,result)=>{
               if(err) throw err;
               else res.render('BE/final',{result:result,navOnly:true})
           })
