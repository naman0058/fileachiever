...
if (!secretkey || !process.env.CCAVENUE_MERCHANT_ID || !process.env.CCAVENUE_WORKING_KEY) {
  console.warn('[verify.js] JWT_SECRET, CCAVENUE_MERCHANT_ID, CCAVENUE_WORKING_KEY must be set in .env for payment features.');
}
...
function shopAuthenticationToken(req, res, next) {
  ...
  if (req.session && req.session.shopkeeper) {
    req.categories = true; // if you really need this flag downstream
    return next();
  }
